...
public:
    ...
    SymbolInfo* get_child_At_fixed_position(int i)
    {
        return children[i];
    }
    SymbolInfo(const SymbolInfo &s)
    {
        strcpy(Name, s.Name);
        strcpy(Type, s.Type);
        strcpy(vtype, s.vtype);
        strcpy(rtype, s.rtype);
        isarray=s.isarray;
        isfunction=s.isfunction;
        isDefi=s.isDefi;
       // parameters= new vector<SymbolInfo*>;
    ...
    void set_start_line(int i)
    {
        start_line=i;

    }
    void set_end_line(int i)
    {
        end_line=i;
    ...
    const char* get_vtype ()const
    {
        return vtype;
    }
    const char* get_rtype ()const
    {
        return rtype;
    }
    ...
    void set_vtype (const char* v)
    {
        vtype=new char[strlen(v)+1];
        strcpy(vtype, v);
    }
    void set_rtype (const char *r)
    {
      // int l= r.length();
       

       rtype=new char[strlen(r)+1];
       //rtype=r.c_str();
       strcpy(rtype,r);
    }
   ...
   /* void set_plist(vector<SymbolInfo*>* p)
    {
        children=p;
    }*/
    vector<SymbolInfo*> get_plist()
    {
        return children;
    }
    void set_parameter_counter(int i){
        parametercounter=i;
    }
    int get__parameter_counter()
    {
    ...
    void set_isarray(bool flag)
    {
        isarray=flag;
    }
    void set_isfunction(bool flag)
    {
        isfunction=flag;
    }
    void set_isDefi(bool flag)
    {
        isDefi=flag;
    }
    
    void addChildren(SymbolInfo *ch) { 
        children.push_back(ch); 
        //start_line = ch->start_line;
        //end_line = ch->end_line;
        if (children.size() > 0)  isleaf= true;
    }
    void add_parameter(string Type_name)
    {
        paramsname.push_back(Type_name);
    }
...
public:
    ...
    // bool Insert_with_data(SymbolInfo *sym, string d)
    // {
    ...
    SymbolInfo* Look_up(const char* name)const
    {
        /* if(Is_empty())
         {
             cout<<"TAble is empty..";
...
public:
    ...
    {
       // cout << "Inside enter scope" << endl;
        ScopeTable* new_scope=new ScopeTable(Scope,bucket_size);
       // cout << "scope table new called" << endl;
        Scope=new_scope;
        //int k= Scope->get_scope_id_counter();
        //cout<<"k "<<k;
        //Scope->set_scope_id_counter(k++);
        //cout<<"k     "<<k;
       // cout << "before logging creation" << endl;
        out<<'\t'<<"ScopeTable# "<<Scope->get_scope_id()<<" created" << endl;
        //cout << "logged" << endl;
    ...
    bool Insertion_with_data_type(const char* name, const char* type)
    {

        cout<<"insert   with datat   "<<name<<endl;
        if(Scope!=nullptr)
        {
            SymbolInfo* sym=new SymbolInfo(name, type);
            // return Scope->get_parent_scope();
            return Scope->Insert(sym);
        }
        else{
            ScopeTable* new_scope=new ScopeTable(Scope,11);
        Scope=new_scope;
        SymbolInfo* sym=new SymbolInfo(name, type);
            // return Scope->get_parent_scope();
            return Scope->Insert(sym);
    ...
    bool Insert_helper(SymbolInfo* s, ofstream& out)
    {
        if(Scope!=nullptr)
        {
            //SymbolInfo* sym=new SymbolInfo(name, type);
            // return Scope->get_parent_scope();
            cout<<s->get_Name()<<" "<<s->get_Type()<<" vvttypp    "<<s->get_vtype()<<endl;
            if(strcmp(s->get_Type(),"ID")!=0){
                return false;
            }
            return Scope->Insert(s);
        }
        return false;
    }
    bool Insert(const char* name, const char* type)
    {
    ...
    SymbolInfo* Look_up(const char* n) const
    {
        ScopeTable *curr= Scope;
        while(curr!=nullptr)
        {
            SymbolInfo *sym=curr->Look_up(n);
            if(sym!=nullptr)
            {
                return sym;
            }
